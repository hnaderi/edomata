...
/** SaaS-aware CQRS repository that writes `tenant_id` and `owner_id` to the
  * states and outbox tables.
  *
  * The tenant/owner information is extracted from the state via
  * [[TenantExtractor]].
  */
private final class SaaSSkunkCQRSRepository[F[_]: Clock, S, N](
    pool: Resource[F, Session[F]],
    state: SaaSQueries.State[S],
    outbox: SaaSQueries.Outbox[N],
    cmds: SaaSQueries.Commands,
    updates: NotificationsPublisher[F],
    handler: SkunkHandler[F][N]
)(using tc: StateModelTC[S], te: TenantExtractor[S], F: Concurrent[F])
  ...
  override def save(
    ...
    val (tenantId, ownerId) = te
      .extract(newState)
      .map((t, o) => (t.value, o.value))
      .getOrElse(("", ""))
      ...
      .use { s =>
        for {
          ...
          _ <- s
            .prepare(state.put)
            .flatMap(
              _.execute((ctx.address, newState, tenantId, ownerId, version))
            )
            .flatMap {
          ...
          _ <- NonEmptyChain.fromChain(events).fold(F.unit) { n =>
            val ns = events.toList
              .map((_, ctx.address, now, ctx.metadata, tenantId))
  ...
  override def notify(
    ...
    val tenantId = "" // notifications without state don't have tenant context
      ...
      .use { s =>
        for {
          ...
          ns = notifications.toList.map(
            (_, ctx.address, now, ctx.metadata, tenantId)
          )
          _ <- s
