...
/** Example application showing the full stack with:
  ...
  *
  * ===Backend CQRS Tables (generated by PGSchema)===
  *
  * {{{
  * // Generate DDL for migration tools:
  * PGSchema.cqrs(PGNaming.prefixed("catalog")).foreach(println)
  *
  * // Output:
  ...
  def backendRes(
    ...
    for
      driver <- Resource.eval(
        SkunkCQRSDriver.from[IO](PGNaming.prefixed("catalog"), pool)
      )
      backend <- Backend
        .builder(ProductService.domain)
        .use(driver)
        .withEventHandler(ProductProjection.handler)
        .build
    yield backend
